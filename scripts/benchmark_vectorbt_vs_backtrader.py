...
def _bench_single(df: pd.DataFrame) -> dict[str, float]:
    params = {
        "rsi_period": 12,
        "up": 53,
        "dn": 36,
...
def _bench_optimize(df: pd.DataFrame, n_jobs: int) -> dict[str, float]:
...
def main() -> None:
    out_dir = Path("reports/vectorbt_performance_20260403")
    ...
    for ds_name, ds in [("synthetic", synth_df), ("xrpusdt_2024_1h", real_df)]:
        single[ds_name] = _bench_single(ds)
    results["single_backtest"] = single

    opt = {}
    for ds_name, ds in [("synthetic", synth_df), ("xrpusdt_2024_1h", real_df)]:
        n1 = _bench_optimize(ds, n_jobs=1)
        n4 = _bench_optimize(ds, n_jobs=4)
        speedup = n1["median_sec"] / max(n4["median_sec"], 1e-9)
        opt[ds_name] = {"n_jobs_1": n1, "n_jobs_4": n4, "speedup_n4_vs_n1": round(float(speedup), 4)}
    ...
    for ds_name, item in results["single_backtest"].items():
        md_lines.append(f"- {ds_name}: median={item['median_sec']:.4f}s")
    md_lines.append("")
    md_lines.append("## Grid Optimization Runtime")
    for ds_name, item in results["optimize_grid"].items():
        md_lines.append(
            f"- {ds_name}: n_jobs=1 median={item['n_jobs_1']['median_sec']:.4f}s, "
            f"n_jobs=4 median={item['n_jobs_4']['median_sec']:.4f}s, speedup={item['speedup_n4_vs_n1']}x"
        )
